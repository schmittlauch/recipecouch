...
exports.search = function(doc, req) {
	...
	events.once('afterResponse', function(info, req, res) {
		if(squery) {
			...
			$.getJSON("/_fti/local/recipes/_design/recipe-couch/recipes",{q: "name:"+squery+',recipedata:'+squery}, function(data, textStatus, jqXHR) {
				if(data[0].total_rows+data[1].total_rows>0) {
					for(var i=0; i<data.length;i++) {
						for(var j=0;j<data[i].rows.length;j++) {
							resultdiv = $("<div>").addClass("result").html('<a href="'+utils.getBaseURL(req)+'/recipe/'+data[i].rows[j].id+'">'+data[i].rows[j].fields.name+'</a> by '+data[i].rows[j].fields.by);
